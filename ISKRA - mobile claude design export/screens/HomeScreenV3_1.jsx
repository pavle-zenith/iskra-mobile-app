  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13, marginTop: 22 }}>
      ...
      <button onClick={onKnowledge} style={{
        ...
        {/* photo as soft texture, blended into the teal surface (not a literal photo) */}
        <div style={{
          position: 'absolute', inset: 0, backgroundImage: `url("${(window.__resources && window.__resources.highlandBg) || 'assets/highland-bg.png'}")`, backgroundSize: 'cover', backgroundPosition: 'center 38%',
          opacity: 0.4, mixBlendMode: 'soft-light', filter: 'saturate(0.7)', pointerEvents: 'none',
        }} />
        {/* gentle depth gradient for legibility */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(255,255,255,0.08) 0%, rgba(8,40,38,0.06) 45%, rgba(8,40,38,0.22) 100%)', pointerEvents: 'none' }} />
        {/* decorative book */}
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 20, fontWeight: 500, color: ISK3.text, letterSpacing: '-0.01em', whiteSpace: 'nowrap' }}>192 dana 6 sati</div>
...
function WeekTracker3({ onCheckIn = () => {}, done = false }) {
  ...
  return (
    <button onClick={onCheckIn} style={{
      position: 'relative', width: '100%', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit', display: 'block',
      border: 'none', borderRadius: 18,
      background: `linear-gradient(158deg, #F0701F 0%, ${ISK3.brand} 58%, #D2581A 100%)`,
      padding: '17px 16px 18px', boxShadow: '0 8px 22px rgba(232,98,26,0.22)', overflow: 'hidden', WebkitTapHighlightColor: 'transparent',
    }}>
      {/* photo as soft texture, blended into the ember surface (not a literal photo) */}
      <div style={{
        position: 'absolute', inset: 0, backgroundImage: `url("${(window.__resources && window.__resources.canyonBg) || 'assets/canyon-bg.png'}")`, backgroundSize: 'cover', backgroundPosition: 'center 45%',
        opacity: 0.42, mixBlendMode: 'soft-light', filter: 'saturate(0.7)', pointerEvents: 'none',
      }} />
      {/* gentle top sheen for depth + keeps numbers legible */}
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(255,255,255,0.10) 0%, rgba(80,40,10,0.06) 45%, rgba(120,60,16,0.20) 100%)', pointerEvents: 'none' }} />
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <span style={{ fontSize: 15.5, fontWeight: 700, color: '#fff', letterSpacing: '-0.01em', textShadow: '0 1px 8px rgba(120,50,10,0.35)', whiteSpace: 'nowrap' }}>Ova nedelja</span>
        <span style={{ fontSize: 12.5, fontWeight: 700, color: 'rgba(255,255,255,0.92)', whiteSpace: 'nowrap', textShadow: '0 1px 8px rgba(120,50,10,0.35)' }}>{done ? '6' : '5'} / 7</span>
      </div>
      <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between' }}>
        {days.map((d) => (
          <div key={d.l} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9 }}>
            <span style={{ fontSize: 11.5, fontWeight: 700, color: '#fff', textShadow: '0 1px 6px rgba(120,50,10,0.4)' }}>{d.l}</span>
            {d.s === 'done' && (
              <div style={{
                width: 30, height: 30, borderRadius: '50%',
                background: '#fff',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                boxShadow: '0 2px 6px rgba(120,50,10,0.28)',
              }}>
                <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
                  <path d="M2.5 7.5 6 11l5.5-7" stroke={ISK3.brand} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            )}
            {d.s === 'celebrate' && (
              <div style={{ position: 'relative', width: 30, height: 30 }}>
                ...
                <div style={{
                  ...
                  background: '#fff',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  boxShadow: '0 2px 6px rgba(120,50,10,0.3)', animation: 'wk3Pop 0.4s cubic-bezier(0.2,0.9,0.3,1.3) both',
                }}>
            ...
            {d.s === 'today' && (
              <div style={{
                ...
                <span style={{ fontSize: 17, fontWeight: 700, color: '#fff', lineHeight: 1 }}>?</span>
              </div>
            )}
            {d.s === 'future' && (
              <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,0.22)', border: '1.5px solid rgba(255,255,255,0.7)', boxShadow: '0 1px 4px rgba(40,24,8,0.2)' }} />
            )}
          </div>
        ))}
      </div>
      <style>{`
        @keyframes subPulse {
          0%, 100% { opacity: 1; box-shadow: 0 0 12px rgba(255,255,255,0.5); }
          50% { opacity: 0.45; box-shadow: 0 0 12px rgba(255,255,255,0); }
        }
        @keyframes wk3Pulse {
...
function Countdown3({ onClick }) {
  const quitRef = React.useRef(Date.now() - ((192 * 86400 + 6 * 3600 + 18 * 60 + 42) * 1000));
...
const QUOTE = {
  ...
  comment: 'Mark Twain je pušio čibuke ceo život. Ali je ovo razumeo. Ti si na danu 192 — streak koji on nikad nije dostigao.',
};
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '58px 20px 10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
          <button onClick={() => setView('settings')} style={{ display: 'flex', alignItems: 'center', gap: 13, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', WebkitTapHighlightColor: 'transparent' }}>
            <div style={{
              ...
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 20, fontWeight: 700, color: ISK3.brandDeep, overflow: 'hidden',
              }}><img src="assets/avatar-pavle.png" alt="Pavle" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} /></div>
            </div>
            <div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 14, fontWeight: 700, color: ISK3.brandDeep }}>192</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            <StatCard3 cat="novac" onClick={() => window.MoneyScreen && setView('money')} icon={(c) => <CoinIcon3 size={20} stroke={c} sw={2} />} value="112.000" label="RSD uštedeno" />
            <StatCard3 cat="cigare" onClick={() => window.CigarettesScreen && setView('cigarettes')} icon={(c) => <CigOffIcon3 size={20} stroke={c} sw={1.9} />} value="1.870" label="cigareta odbijeno" />
          </div>
...
Object.assign(window, { HomeScreenV3_1 });
